...
/**
 * Gemini sends a JSON error body describing *why* a non-2xx happened (bad
 * key, unknown model, malformed request, ...). Reading only `response.status`
 * throws that reason away, leaving the caller stuck guessing from a bare
 * status code alone.
 */
    ...
    async *complete(
      ...
      if (response.status < 200 || response.status >= 300) {
        ...
        yield {
          ...
          error:
            detail === undefined
              ? `Google Gemini returned HTTP ${response.status}`
              : `Google Gemini returned HTTP ${response.status}: ${detail}`,
        }
        return
      }
