...
export function checkProviderConfig(): DoctorCheck {
    ...
    if (fs.existsSync(configPath)) {
      try {
        ...
        if (typeof config['apiKey'] === 'string' && config['apiKey'].length > 0) {
          return { name: 'Provider API key', status: 'PASS', detail: `Configured via ${configPath}` }
        }
      } catch {
  ...
  return { name: 'Provider API key', status: 'WARN', detail: 'No ANTHROPIC_API_KEY set and no config file found' }
}
...
export function checkHiveMindRegistry(): DoctorCheck {
  ...
  if (types.length === expected) {
    return { name: 'HiveMind registry', status: 'PASS', detail: `${types.length}/${expected} agent types loaded` }
  }
  return { name: 'HiveMind registry', status: 'FAIL', detail: `${types.length}/${expected} agent types loaded` }
}
...
export function checkAjnaPipeline(): DoctorCheck {
  ...
  if (existing.length === ajnaFiles.length) {
    return { name: 'Ajna pipeline', status: 'PASS', detail: `All ${ajnaFiles.length} core modules present` }
  }
  return { name: 'Ajna pipeline', status: 'WARN', detail: `${existing.length}/${ajnaFiles.length} core modules found` }
}
...
export function checkPersistenceLayer(): DoctorCheck {
  ...
  if (existing.length === storageFiles.length) {
    return { name: 'Persistence layer', status: 'PASS', detail: `All ${storageFiles.length} storage modules present` }
  }
  return { name: 'Persistence layer', status: 'WARN', detail: `${existing.length}/${storageFiles.length} storage modules found` }
}
...
export function checkAgentLoop(): DoctorCheck {
  ...
  if (existing.length === agentFiles.length) {
    return { name: 'Agent loop', status: 'PASS', detail: `All ${agentFiles.length} agent modules present` }
  }
  return { name: 'Agent loop', status: 'FAIL', detail: `${existing.length}/${agentFiles.length} agent modules found` }
}
...
export function checkToolRegistry(workspaceRoot: string): DoctorCheck {
  ...
  const toolFiles = fs.readdirSync(toolsDir).filter((f) => f.endsWith('-tool.ts') && !f.endsWith('.spec.ts'))

  if (toolFiles.length >= 10) {
    return { name: 'Tool registry', status: 'PASS', detail: `${toolFiles.length} tool definitions found` }
  }
  return { name: 'Tool registry', status: 'WARN', detail: `Only ${toolFiles.length} tool definitions found` }
}
...
export function checkTuiLayer(): DoctorCheck {
  ...
  if (existing.length === tuiFiles.length) {
    return { name: 'TUI layer', status: 'PASS', detail: `All ${tuiFiles.length} TUI modules present` }
  }
  return { name: 'TUI layer', status: 'WARN', detail: `${existing.length}/${tuiFiles.length} TUI modules found` }
}
