 ...
 * the cross-port "open CodeMind Chat" link — the Agent view is now just
...
export function renderDashboardViewHtml(): string {
  return `<section data-view="dashboard" class="app-view">
    ...
    <section id="dashboard-status" class="muted">Loading CodeMind runtime status...</section>
  </section>`
}
    ...
    async function loadDashboardStatus() {
      ...
      try {
        const response = await fetch('/api/status', {
          cache: 'no-store',
          headers: appState.codemindKey ? { authorization: 'Bearer ' + appState.codemindKey } : {},
        });
        ...
        statusEl.innerHTML = '<p class="muted">Connect with your CodeMind API key in Settings to load detailed status, or ' +
