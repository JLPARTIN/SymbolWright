...
export class FakeLiveReadClient implements RuntimeLiveReadClient {
  ...
  async getPullRequestEvidence(_owner: string, _repo: string, _prNumber: number): Promise<GitHubPrEvidence> {
  ...
  async getWorkflowEvidence(_owner: string, _repo: string, _runId: number): Promise<GitHubCiEvidence> {
  ...
  async getRepositoryFile(_owner: string, _repo: string, path: string, ref: string): Promise<RepositoryFileResult> {
