...
describe('AGENT-KERNEL-01 planner', () => {
  ...
  it('defaults to a deterministic plan with operator checkpoints', () => {
    ...
    expect(decision.workflowSteps.every((step) => step.allowedToMutate === false)).toBe(
      true,
    );
  });
  ...
  it('adds patch proposal planning only when explicitly allowed', () => {
    ...
    expect(planOnly.workflowSteps.some((step) => step.kind === 'patch-proposal')).toBe(
      false,
    );
    expect(patchPlan.workflowSteps.some((step) => step.kind === 'patch-proposal')).toBe(
      true,
    );
  });
  ...
  it('includes A-O-S source lineage in the planning decision', () => {
    const decision = planAgentKernel01(
      makeRequest({ requestedRoles: ['memory-auditor'] }),
    );

    expect(decision.sourceLineage.some((line) => line.includes('X1YA0I-A-O-S'))).toBe(
      true,
    );
    expect(decision.workflowSteps.some((step) => step.stepId === 'ak-01-memory-lineage-audit')).toBe(
      true,
    );
  });
});
