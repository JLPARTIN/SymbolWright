...
export type AgentKernelProviderRouteType =
  (typeof AGENT_KERNEL_PROVIDER_ROUTE_TYPES)[number];
...
export const AGENT_KERNEL_PROVIDER_ROUTE_SEVERITIES = [
  'INFO',
  'WARN',
  'BLOCK',
] as const;
...
export function planAgentKernelProviderRoute(
  ...
  if (packet.providerInvoked) {
    findings.push(
      makeFinding('PACKET_ALREADY_INVOKED', 'BLOCK', 'Context packet must not be routed after provider invocation.'),
    );
  }
