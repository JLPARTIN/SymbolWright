...
export const AGENT_KERNEL_CONTEXT_PRIORITIES = [
  'CRITICAL',
  'HIGH',
  'MEDIUM',
  'LOW',
] as const;
export type AgentKernelContextPriority =
  (typeof AGENT_KERNEL_CONTEXT_PRIORITIES)[number];
...
export type AgentKernelContextPacketSection =
  (typeof AGENT_KERNEL_CONTEXT_PACKET_SECTIONS)[number];
...
export function buildAgentKernelContextPacket(
  ...
  pushIfAllowed(items, omittedSections, maxSections, {
    ...
    content: planning.doctrineNotes.length > 0
  ...
  pushIfAllowed(items, omittedSections, maxSections, {
    ...
    content: planning.doctrineNotes.length === 0 ? 'No doctrine notes emitted.' : planning.doctrineNotes.join('\n'),
  });
