...
export const AGENT_KERNEL_ROUTE_PREFLIGHT_SEVERITIES = [
  'INFO',
  'ASK',
  'BLOCK',
] as const;
...
function isExternalRoute(routeType: AgentKernelProviderRouteType): boolean {
  return routeType === 'LIGHTWEIGHT_REASONING' ||
...
export function preflightAgentKernelRouteExecution(
  ...
  if (!routePlan.providerRouteReady) {
    findings.push(
      makeFinding('ROUTE_NOT_READY', 'BLOCK', 'Provider route plan is not ready for execution preflight.'),
    );
  }

  if (routePlan.providerInvoked) {
    findings.push(
      makeFinding('PROVIDER_ALREADY_INVOKED', 'BLOCK', 'Route plan indicates a provider was already invoked.'),
    );
  }
  ...
  if (policy.blockOnRouteWarnings && routePlan.findings.some((finding) => finding.severity === 'WARN')) {
    findings.push(
      makeFinding('ROUTE_FINDINGS_BLOCKING', 'BLOCK', 'Route plan contains warnings blocked by execution policy.'),
    );
  }
  ...
  if (accepted) {
    findings.push(
      makeFinding('PREFLIGHT_ACCEPTED', 'INFO', `Route type ${routePlan.routeType} passed execution preflight.`),
    );
  ...
  rationale.push('Provider invocation, repository mutation, and command execution remain outside this block.');
