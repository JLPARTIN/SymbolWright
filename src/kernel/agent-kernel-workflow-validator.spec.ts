...
describe('AGENT-KERNEL-02 workflow validator', () => {
  ...
  it('rejects empty workflows', () => {
    ...
    expect(report.findings.map((finding) => finding.code)).toContain('MISSING_OPERATOR_CHECKPOINT')
  })
  ...
  it('rejects operator checkpoints that do not point to approval checkpoint steps', () => {
    ...
    expect(report.valid).toBe(false)
    expect(report.findings.map((finding) => finding.code)).toContain('INVALID_OPERATOR_CHECKPOINT')
  })
})
