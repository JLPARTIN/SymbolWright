...
export type AgentKernelSkillProposalStatus = (typeof AGENT_KERNEL_SKILL_PROPOSAL_STATUSES)[number]
...
export function createAgentKernelSkillProposal(input: {
  readonly proposalId: string
  readonly proposedSkill: AgentKernelSkillDeclaration
  readonly proposedBy: string
  readonly rationale: string
  ...
  if (skill.riskClass === 'HIGH' || skill.riskClass === 'CRITICAL') {
    quarantineReasons.push(
      'High-risk and critical skills require operator review before registry promotion.',
    )
  }
