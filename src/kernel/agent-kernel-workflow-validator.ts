...
export type AgentKernelWorkflowFindingCode = (typeof AGENT_KERNEL_WORKFLOW_FINDING_CODES)[number]
...
export function validateAgentKernelWorkflow(
  ...
  if (decision.workflowSteps.length === 0) {
    findings.push(
      makeFinding(
        'EMPTY_WORKFLOW',
        'CRITICAL',
        'Agent Kernel workflow must contain at least one step.',
      ),
    )
  }
  ...
  for (const profile of decision.roleProfiles) {
    if (!knownRoles.includes(profile.role)) {
      findings.push(makeFinding('UNKNOWN_ROLE', 'HIGH', `Unknown role profile: ${profile.role}.`))
    }
    ...
    if (hasDuplicateStepId(step, decision.workflowSteps)) {
      findings.push(
        makeFinding(
          'DUPLICATE_STEP_ID',
          'HIGH',
          `Duplicate workflow step id: ${step.stepId}.`,
          step.stepId,
        ),
      )
    }
    ...
    if (!knownRoles.includes(step.role)) {
      findings.push(
        makeFinding(
          'UNKNOWN_ROLE',
          'HIGH',
          `Unknown workflow step role: ${step.role}.`,
          step.stepId,
        ),
      )
    }
    ...
    if (!knownStepKinds.includes(step.kind)) {
      findings.push(
        makeFinding(
          'UNKNOWN_STEP_KIND',
          'HIGH',
          `Unknown workflow step kind: ${step.kind}.`,
          step.stepId,
        ),
      )
    }
    ...
    if (typeof step.skillId === 'string' && !knownSkillIds.includes(step.skillId)) {
      findings.push(
        makeFinding(
          'UNKNOWN_SKILL',
          'HIGH',
          `Unknown workflow step skill: ${step.skillId}.`,
          step.stepId,
        ),
      )
    }
