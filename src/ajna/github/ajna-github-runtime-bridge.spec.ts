...
describe('Ajna GitHub runtime bridge', () => {
  it('maps GitHub read context into an Ajna review request', () => {
    const ajnaRequest = mapGithubContextToAjnaReviewRequest(
      'ajna-github-10',
      makeGithubContext(),
      {
    ...
    expect(ajnaRequest.changedFiles).toEqual([
      'src/ajna/github/ajna-github-runtime-bridge.ts',
    ]);
  ...
  it('passes approved GitHub PR reads through the runtime boundary', async () => {
    ...
    expect(result.ajnaReviewRequest.subject.repository).toBe(
      'JLPARTIN/JLPARTIN-CodeMind',
    );
  ...
  it('blocks unapproved GitHub PR reads at the runtime boundary', async () => {
    await expect(
      ...
      'GitHub PR read adapter did not pass the CodeMind runtime boundary.',
    );
  });
});
