...
describe('buildAjnaRepoScanProfile', () => {
  ...
  it('blocks when required source and TypeScript proof signals are missing', () => {
    ...
    expect(
      profile.signals.filter((signal) => signal.status === 'FAIL').map((signal) => signal.id),
    ).toEqual(['source.root', 'source.typescript', 'tooling.typescript'])
  ...
  it('needs attention when optional proof guardrails are incomplete', () => {
    ...
    expect(
      profile.signals.filter((signal) => signal.status === 'WARN').map((signal) => signal.id),
    ).toEqual(['tests.present', 'tooling.lint', 'tooling.format'])
    expect(profile.recommendations).toContain(
