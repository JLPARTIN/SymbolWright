...
export const AJNA_RISK_SYNTHESIS_BLOCK_ID = 'CODEMIND-AJNA-REVIEW-03' as const;
export const AJNA_RISK_SYNTHESIS_PR_ID = 'PR-CM-AJNA-03' as const;
export const AJNA_RISK_SYNTHESIS_PHASE_ID = 'CODEMIND-AJNA-03' as const;

export const AJNA_RISK_LEVELS = [
  'LOW',
  'MODERATE',
  'HIGH',
  'CRITICAL',
...
export interface AjnaRiskSynthesisInput {
  ...
  /** Impact level from repo context (e.g. HIGH, MODERATE, LOW). */
...
export function synthesizeAjnaRisk(
  input: AjnaRiskSynthesisInput,
): AjnaRiskSynthesis {
  ...
  if (
    ...
    proofBundle.invalidProofDomains.forEach((d) =>
      explanation.push(`  invalid: ${d}`),
    );
  } else if (
    proofBundle.blockingProofDomains.length > 0 ||
    blockingFindings.length > 0
  ) {
    riskLevel = 'BLOCKED';
    explanation.push('BLOCKED: one or more proof domains are blocked.');
    proofBundle.blockingProofDomains.forEach((d) =>
      explanation.push(`  blocked: ${d}`),
    );
    blockingFindings.forEach((f) => explanation.push(`  finding: ${f}`));
