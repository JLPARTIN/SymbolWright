...
export function synthesizeAjnaRisk(
  ...
  if (
    ...
    proofBundle.invalidProofDomains.forEach((domain) =>
      explanation.push(`invalid: ${domain}`),
    );
  } else if (
    ...
    proofBundle.blockingProofDomains.forEach((domain) =>
      explanation.push(`blocked: ${domain}`),
    );
