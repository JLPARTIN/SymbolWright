...
export function createAjnaSavedReviewRecord(input: AjnaSavedReviewInput): AjnaSavedReviewRecord {
  return {
    id: input.id,
    savedAt: input.savedAt,
    label:
      input.label ?? `${input.review.repository}#${input.review.pullRequestNumber ?? 'unknown'}`,
    review: input.review,
  }
}
...
export function upsertAjnaSavedReview(
  ...
  return [...filtered, next].sort((left, right) => left.savedAt.localeCompare(right.savedAt))
}
