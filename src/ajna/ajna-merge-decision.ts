...
export const AJNA_MERGE_DECISION_BLOCK_ID = 'CODEMIND-AJNA-REVIEW-04' as const;
export const AJNA_MERGE_DECISION_PR_ID = 'PR-CM-AJNA-04' as const;
export const AJNA_MERGE_DECISION_PHASE_ID = 'CODEMIND-AJNA-04' as const;
...
export type AjnaMergeDecisionState =
  (typeof AJNA_MERGE_DECISION_STATES)[number];

const OPERATOR_REVIEW_RISK_LEVELS: ReadonlySet<AjnaRiskLevel> = new Set([
  'HIGH',
  'CRITICAL',
]);
...
const READY_RISK_LEVELS: ReadonlySet<AjnaRiskLevel> = new Set([
  'LOW',
  'MODERATE',
]);
...
export interface AjnaMergeDecisionInput {
  ...
  /** Set true if an operator gate requires human approval. */
...
export function buildAjnaMergeDecision(
  input: AjnaMergeDecisionInput,
): AjnaMergeDecision {
  ...
  if (BLOCKED_RISK_LEVELS.has(riskSynthesis.riskLevel)) {
    ...
    if (proofBundle.blockingProofDomains.length > 0) {
      reasons.push(
        `Blocked domains: ${proofBundle.blockingProofDomains.join(', ')}.`,
      );
    }
    if (proofBundle.invalidProofDomains.length > 0) {
      reasons.push(
        `Invalid domains: ${proofBundle.invalidProofDomains.join(', ')}.`,
      );
    }
    if (proofBundle.missingProofDomains.length > 0) {
      reasons.push(
        `Missing domains: ${proofBundle.missingProofDomains.join(', ')}.`,
      );
    }
    ...
    state = 'MERGE_READY';
    reasons.push(
      `All proof domains ready. Risk level: ${riskSynthesis.riskLevel}.`,
    );
  } else {
    state = 'NOT_READY';
