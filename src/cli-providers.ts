...
export function renderProvidersCommand(args: readonly string[] = []): string {
  ...
  if (subcommand === 'health' || subcommand === 'status') {
    return [
      ...
      `Fallbacks:       ${redacted.fallbackProviders.length === 0 ? 'none' : redacted.fallbackProviders.join(', ')}`,
      '',
      ...statuses.map(
        (status) =>
  ...
  return [
    ...
    ...redacted.providers.map(
      (provider) =>
        `  ${provider.id.padEnd(14)} key=${provider.apiKey.padEnd(10)} enabled=${String(provider.enabled).padEnd(5)} model=${provider.defaultModel ?? 'not configured'}`,
    ),
  ].join('\n')
}
