...
export function buildSymbolWrightKernelTraceProofReport(
  ...
  const matchingFrames = input.evidenceFrames.filter((f) => f.executionId === input.executionId)
  ...
  const validFrameBlockIds = new Set(
    replayErrors.length === 0 ? matchingFrames.map((f) => f.blockId) : [],
  )

  const coveredBlockIds = input.requiredBlockIds.filter((id) => validFrameBlockIds.has(id))
  const missingBlockIds = input.requiredBlockIds.filter((id) => !validFrameBlockIds.has(id))
