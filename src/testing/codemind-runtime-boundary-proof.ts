export const CODEMIND_RUNTIME_BOUNDARY_PROOF_BLOCK_ID =
  'CODEMIND-PROOF-HARNESS-06' as const;
...
function collectFlagViolations(
  flags: CodemindRuntimeBoundaryFlags,
): readonly string[] {
...
export function buildCodemindRuntimeBoundaryProofReport(
  ...
  const missingGates = input.requiredGates.filter(
    (gate) => !presentGateSet.has(gate),
  );
