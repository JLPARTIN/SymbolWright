...
export function evaluateSandboxPolicy(
  ...
  if (request.mode === 'run' && !runner.capabilities.run) {
    return { allowed: false, reason: `Runner ${runner.id} does not support run mode.` }
  }
  if (request.mode === 'compile' && !runner.capabilities.compile) {
    return { allowed: false, reason: `Runner ${runner.id} does not support compile mode.` }
  }
  if (request.mode === 'test' && !runner.capabilities.test) {
    return { allowed: false, reason: `Runner ${runner.id} does not support test mode.` }
  }
  if (request.repository !== undefined && !runner.capabilities.repository) {
    return { allowed: false, reason: `Runner ${runner.id} does not support repository execution.` }
  }

  if (context.mode === 'READ_ONLY') {
    return { allowed: false, reason: 'READ_ONLY may list runtime inventory but cannot execute code.' }
  }
  if (context.mode === 'PROPOSAL_ONLY' || context.mode === 'PLAN_ONLY') {
    return { allowed: false, reason: `${context.mode} may propose an execution plan but cannot run it.` }
  }
  ...
  if (runner.trustClass === 'guarded-host') {
    ...
    return { allowed: true, reason: 'Approved guarded-host execution; not a strong sandbox.' }
  }
  ...
  return { allowed: true, reason: `${runner.trustClass} execution allowed by runtime policy.` }
}
