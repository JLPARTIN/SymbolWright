...
CodeMind Skills are file-based, reusable coding-agent workflows. They use the same basic shape as Claude Code skills while staying CodeMind-native: every shell command, forked subagent run, and mutation still goes through CodeMind runtime policy, checkpointing, and tool isolation.

## Skill locations

CodeMind loads skills from these locations, in this precedence order:

1. `.codemind/skills/<skill-name>/SKILL.md` — native project skills
...
4. bundled CodeMind skills — available out of the gate

Project skills override bundled skills with the same command name. This lets a repository customize `repo-forensics`, `run`, or `verify` without changing CodeMind internals.
...
```bash
CODEMIND_DISABLE_SKILL_SHELL_EXECUTION=1 codemind skill run summarize-changes
```
...
- `${CODEMIND_SESSION_ID}` and `${CLAUDE_SESSION_ID}`
- `${CODEMIND_SKILL_DIR}` and `${CLAUDE_SKILL_DIR}`
- `${CODEMIND_PROJECT_DIR}` and `${CLAUDE_PROJECT_DIR}`
...
CodeMind does not bypass its runtime policy for these commands. Dynamic context uses the existing sandboxed bash execution path. If policy blocks shell execution, the injected section is replaced with a clear blocked message instead of silently running.
...
CodeMind ships these bundled Skills v1 entries:
...
Skills do not bypass write gates. If a skill causes a file mutation through normal CodeMind tools, the checkpoint system still snapshots touched files before mutation.
