...
**`symbolwright agent`**: `src/cli.ts:99-101` → `src/cli-agent.ts`, runs the tool loop directly in the terminal.
**`symbolwright mcp-server`**: `src/cli.ts:309-311` → `src/cli-mcp-server.ts:55` → `src/mcp/mcp-server.ts`, stdio JSON-RPC, defaults to `READ_ONLY` (deliberately narrower than the platform default) since it is a background process any connected MCP client can drive.
